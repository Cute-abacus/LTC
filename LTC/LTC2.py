...
# '''加载数据'''
# def load_abilene():
#     A = sio.loadmat('subData.mat')
#     return A[:,:,:trainPiece] # 用这组数据算误差

def load_abilene():
    # import Abilene dataset
    data_dir = os.path.join(os.getcwd(), '../data')
    mat_fname = os.path.join(data_dir, 'oldData.mat')#p2p1953
...
class TensorDecompCP(object):
    ...
    def random_mask(cls, n, p):
        ...
        """
        size = math.prod(n)
        # size = reduce(np.operator.mul, n, 1)
        mask = np.zeros(size)
        K = int(size * p)  # K 1s
        mask[:K] = 1
