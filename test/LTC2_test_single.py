...
from LTC.StoreMat import store, load
...
def CP_test(sample_rate, rank):
    ...
    TD_obj = LTC2.TensorDecompCP(T, mask, rank, iters=10)

    TD_obj.run()  # perform CP decomposition
    data = {'mask': mask}
    store('../data/p2p1953mask_6%_5.mat', data)
    # tensor factor matrices: [A, B, C]
    factors = TD_obj.factors
    end = time.perf_counter()
    print(f"Module CP_test took {end - start :0.4f} seconds")
    return TD_obj

@time_it
def LTC_test(TD_obj, h, q, alpha, sample_rate, rank, times, ap_cap, sub_rank, return_dict, it, dis_mode):
    ...
    data = {'X': T, 'Xhat': LTC_recovered}
    store('../data/p2p1953_6%_5.mat', data)
...
if __name__ == "__main__":
    ...
    # h
    for i, v in np.ndenumerate(range(1, 31, 1)):
        ...
        for run in range(runs):
            ...
            params_copy['it'] = run
            job = LTC_test(TD_obj, **params_copy)
            with open(path, 'a', newline='') as f:
                wr = csv.writer(f, delimiter=',')
                wr.writerow(job)
